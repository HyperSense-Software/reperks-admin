...
const AddPhoneComponent = () => {
  ...
  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    try {
      ...
      router.push('/dashboard/2FA/confirmCode?confirm=sms');
    } catch (err: unknown) {
    ...
    <div className="flex flex-col content-center justify-center gap-8 self-center">
      <Card className={`w-dvw max-w-[744px]`}>
        <CardHeader>
          ...
          <CardDescription>Write your phone number in the input.</CardDescription>
        </CardHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="max-w-sm gap-1.5">
                    ...
                    <FormControl>
                      <Input type="phone" id="phone" placeholder="Phone number" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <CardContent className="flex flex-row justify-end gap-6">
              ...
              <Button type="submit" className={form.getValues('phone') !== '' ? `` : `bg-gray-400`}>
                Send Code
              </Button>
            </CardContent>
          </form>
