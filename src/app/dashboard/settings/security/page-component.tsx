...
type AuthMFAType = 'SMS' | 'TOTP' | null;
    ...
    try {
      switch (type) {
        case 'SMS':
          ...
          setMfaEnabledType(mfaEnabledType.filter((AuthMFAType) => AuthMFAType !== 'SMS'));
        ...
        case 'TOTP':
          ...
          setMfaEnabledType(mfaEnabledType.filter((AuthMFAType) => AuthMFAType !== 'TOTP'));
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      <div className={`flex w-full flex-col gap-4`}>
        <h1 className={'font-semibold text-black'}>Two-factor authentication</h1>
        <p className={`font-normal text-gray-500`}>
          Two-factor authentication is an enhanced security measure. Once enabled. you’ll be
          required to give two types of identification when you log into HSS Admin.
        </p>
      </div>
      <div className={`flex w-full flex-col gap-6`}>
        <Card>
          <CardHeader className="gap-2">
            ...
            <CardDescription>
              By enabling this feature you will have to register your phone and verify it with the
              code received via <span className="font-bold">SMS</span>.
              <br />
              Later on, every time you login into the app you will have to additionally verify
              yourself by submitting a code that you will receive via{' '}
          ...
          {mfaEnabledType && mfaEnabledType.includes('SMS') ? (
            ...
            <CardFooter className="flex flex-row justify-end py-6">
              <Button asChild type="button">
                <Link href={'/dashboard/2FA/sms/addPhone'}>Set up 2FA with SMS</Link>
              </Button>
            </CardFooter>
          )}
        </Card>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              By enabling this feature you will have to scan a QR code with your phone in order to
              add the TOTP to an authenticator app like{' '}
          ...
          {mfaEnabledType && mfaEnabledType.includes('TOTP') ? (
            ...
            <CardFooter className="flex flex-row justify-end py-6">
              <Button asChild type="button">
                <Link href={'/dashboard/2FA/totp/qrCode'}>Set up 2FA with TOTP</Link>
              </Button>
            </CardFooter>
          )}
        </Card>
